...
export function activate(context: vscode.ExtensionContext) {
    ...
    async function executeSelectedPython(editor: vscode.TextEditor, moveCursor: boolean, wholeProgram: boolean = false): Promise<ExecutionResult> {
        ...
        const pylotConfig = vscode.workspace.getConfiguration('pylot');
        const command = {
            code: JSON.stringify(code),
            filename: editor.document.fileName,
            start_line: executionSelection.start.line + 1,
            progress: pylotConfig.get<boolean>('enableForLoopLiveUpdates', true),
            watch: pylotConfig.get<boolean>('enableWatchComments', true),
